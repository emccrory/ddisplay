...
public class MyColorSliderUI extends BasicSliderUI {
	...
	public static final void main( String [] args ) {
		...
		System.out.println(Color.white + ", " + Color.gray.brighter() + ", " + myWhite + ", "
				+ myGray);
		slider.setUI(new MyColorSliderUI(slider, new Color [] { Color.red, Color.white,
				Color.orange, Color.white, Color.blue }, new int [] { 1, 5, 9, 22, 23 }));
	...
	public MyColorSliderUI( JSlider slider, Color [] c, int [] labels ) {
		super(slider);
		assert (c != null);
		colors = c;
	...
	public void paintTrack( Graphics g ) {
		...
		g2d.setPaint(Color.black);
		g2d.drawLine(t.x, (int) Math.round(t.y + inc / 2.0), t.x + t.width - 1,
				(int) Math.round(t.y + inc / 2.0));
		int ymax;
		...
		g2d.fillRect(t.x, 1 + (int) Math.round(ymax + inc / 2.0), t.width,
