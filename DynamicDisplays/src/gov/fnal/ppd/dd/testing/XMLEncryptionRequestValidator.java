...
public class XMLEncryptionRequestValidator {
	...
	private final static Logger			logger	= LoggerFactory.getLogger(XMLEncryptionRequestValidator.class);

	private final XMLSignatureFactory	factory;

	private final KeySelector			keySelector;

	public XMLEncryptionRequestValidator(final KeySelector keySelector) {
	...
	public static class Result {
		...
		public Result(final String statusCode, final String message) {
			this.valid = false;
			this.signature = null;
			this.keySelectorResult = null;
	...
	public Result validate(final Document doc) throws Exception {
		...
		if (nl.getLength() == 0) {
			return new Result("VALIDATE_NO_SIGNATURE_DATA", "No signature data found");
		}
