...
import gov.fnal.ppd.dd.display.client.WrapperType;
...
/**
 * Copied from DisplayAsConnectionToFirefox. 
 * 
 * 
 * 
...
public class DisplayAsConnectionThroughSelenium extends DisplayControllerMessagingAbstract {
	...
	public void initiate() {
		...
		new Thread() {
			public void run() {
				println(getClass(), ".initiate(): Here we go! display number=" + getVirtualDisplayNumber() + " ("
	...
	private void setRevertThread() {
		...
		if (revertThread == null) {
			revertThread = new Thread("RevertContent" + getMessagingName()) {
				...
				public void run() {
					long increment = 15000L;
					println(DisplayAsConnectionThroughSelenium.class, ".setRevertThread(): Revert thread started.");
					...
					while (true) {
						...
						println(DisplayAsConnectionThroughSelenium.class, ".localSetContent():" + browserInstance.getInstance()
						...
						try {
							...
							println(DisplayAsConnectionThroughSelenium.class, ".setRevertThread():" + browserInstance.getInstance()
			...
			println(DisplayAsConnectionThroughSelenium.class, ".setRevertThread(): revert thread is already running.");
		}
	}
		...
		Thread t = new Thread("RemoveExtraFrame") {
			...
			public void run() {
				...
				while (removeFrame[frameNumber] == false && frameRemovalTime[frameNumber] > 0) {
					println(DisplayAsConnectionThroughSelenium.class, browserInstance.getInstance() + " Continuing; frame off in  "
